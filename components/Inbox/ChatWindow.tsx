...
const ChatWindow: React.FC<ChatWindowProps> = ({ conversation }) => {
  ...
  useEffect(() => {
    const syncThread = async () => {
      ...
      try {
        // Corrected call: Passing page.id to identify direction correctly
        const metaMsgs = await fetchThreadMessages(conversation.id, page.id, page.accessToken);
        
        for (const msg of metaMsgs) {
          const alreadyExists = messages.find(m => m.id === msg.id);
          
          if (!alreadyExists) {
            // Check for a local optimistic match to replace (same text from current agent sent in the last 30 seconds)
            const localMatch = messages.find(m => 
              m.conversationId === msg.conversationId && 
              m.senderId === currentUser?.id &&
              m.text === msg.text &&
              m.id.startsWith('msg-') // Local IDs generated by Date.now()
            );

            // If we find a local optimistic version and this is an outgoing message from the page
            if (localMatch && !msg.isIncoming) {
              // Note: Ideally we would 'replace' the message, but with the current simple store, 
              // just adding it works as syncThread runs and we can't easily 'delete' from this context without complex store updates.
              // For now, we rely on the fact that fetchThreadMessages returns the official Meta ID which we then persist.
    ...
    // Polling sync every 10 seconds
    const poll = setInterval(syncThread, 10000);
    return () => clearInterval(poll);
  }, [conversation.id, pages, currentUser]);
